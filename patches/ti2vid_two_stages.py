...
class TI2VidTwoStagesPipeline:
    ...
    def __init__(
        ...
        stage_2_checkpoint_path: str | None = None,
    ):
        ...
        # Stage 2: use pre-fused checkpoint (no runtime LoRA fusion) or runtime fusion
        if stage_2_checkpoint_path:
            self.stage_2 = DiffusionStage(
                stage_2_checkpoint_path,
                self.dtype,
                self.device,
                loras=(),  # LoRA already baked into checkpoint
                quantization=quantization,
                registry=registry,
                torch_compile=torch_compile,
            )
